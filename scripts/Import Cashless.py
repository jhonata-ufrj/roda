...
df = pandas.read_csv(r"C:./datasets./detached.csv",";",index_col=0)
...
# df = df.loc[df['occurred_at']>=datetime(2021, 3, 1)]
...
class VMConnection(object):
  ...
  # Diretórios
  DATASET_BASEDIR = "./dataset/"#Vendas/" #% datetime.now().strftime('%d%m%Y%H%M%S')
  ...
  def getPurchases(self, save=True, **kwargs) -> list:
    ...
    '''
    df = pandas.read_csv(r"./datasets/detached.csv",";",index_col=0)
...
purchases = VMConnection().getPurchases()